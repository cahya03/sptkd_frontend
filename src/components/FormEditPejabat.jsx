...
const FormEditPejabat = () => {
  ...
  //UseEffect untuk fetch data pejabat by id
  useEffect(() => {
    const getPejabatById = async () => {
      try {
        const response = await axios.get(`${base_url}/api/pejabat/${id}`);
        const { pejabat_nama, pejabat_nip, pejabat_jabatan } = response.data;
        setFormData({ pejabat_nama, pejabat_nip, pejabat_jabatan });
      } catch (error) {
        if (error.response) {
  ...
  return (
    <div>
      ...
      <h2 className="text-2xl mb-4 font-semibold">Edit Pejabat</h2>
...
export default FormEditPejabat;
